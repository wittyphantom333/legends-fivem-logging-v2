...
RegisterNetEvent("legendsSystems:GetNotLogged")
AddEventHandler("legendsSystems:GetNotLogged", function(tab)
...
AddEventHandler("legendsSystems:ClientCreateScreenshot", function(args)
	exports["screenshot-basic"]:requestScreenshotUpload(
		"https://discord.com/api/webhooks/" .. args.url,
		"files[]",
		function(data)
			...
			if resp.attachments then
				...
				TriggerServerEvent("legendsSystems:ScreenshotCB", args)
			end
		end
	)
end)
		...
		elseif not IsPedShooting(playerped) and fireCount ~= 0 and timeout ~= 0 then
			...
			if fireCount ~= 0 and timeout == 0 then
				if not ClientTables.WeaponNames[tostring(fireWeapon)] then
					TriggerServerEvent("legendsSystems:playerShotWeapon", "`Undefined`", fireCount)
					return
				end
				...
				if isLoggedWeapon then
					TriggerServerEvent(
						...
						ClientTables.WeaponNames[tostring(fireWeapon)],
						fireCount
					)
				end
				fireCount = 0
		...
		if IsEntityDead(iPed) then
			if not hasRun then
				...
				TriggerServerEvent(
					...
					{ ["rsn"] = DeathReason, ["kil"] = GetPlayerServerId(killer) }
				)
			end
		else
			Wait(200)
...
CreateThread(function()
	...
	while true do
		...
		end -- need to know the health if we gonna log the change of it ;)
		if health < GetEntityHealth(PlayerPedId()) then
			health = GetEntityHealth(PlayerPedId())
		end -- Just to make sure we don't log when healt gets added
		if health > GetEntityHealth(PlayerPedId()) then
			newHealth = GetEntityHealth(PlayerPedId())
			TriggerServerEvent("legendsSystems:PlayerDamage", math.floor((health - newHealth) / 2))
			health = newHealth
			Wait(500)
		else
			Wait(1000)
