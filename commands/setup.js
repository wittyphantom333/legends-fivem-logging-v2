module.exports = {
	...
	async execute(message) {
		...
		if(content.toLowerCase().startsWith(`${client[1].config.prefix}legendsDiscordLogs setup`)){
			...
			await channel.send('**Starting legendsDiscordLogs Setup!**\n*ETA: 30 seconds*').then(msg => {
				response = msg
			})
			ids = ''
			info = {
			...
			if(x){
				if(!guild.channels.cache.find(cc => cc.name === `📢・legendsSystems-announcements`)){
					await guild.channels.create("📢・legendsSystems-announcements", {
							type: 'GUILD_TEXT',
			...
			setTimeout(() => {
				response.edit(`**Starting legendsDiscordLogs Setup!**\n*ETA: 15 seconds*`)
			}, 15000)
			setTimeout(() => {
