...
function GetPlayerDetails(src, channel)
	...
	if cfgFile["postals"] and GetResourceKvpString("legendsDiscordLogs:" .. channel:lower() .. ":postal") ~= "true" then
	...
	if
		cfgFile["discordId"]
		and GetResourceKvpString("legendsDiscordLogs:" .. channel:lower() .. ":discordid") ~= "true"
	then
		if ids.discord then
	...
	if GetConvar("steam_webApiKey", "false") ~= "false" then
		if
			cfgFile["steamId"]
			and GetResourceKvpString("legendsDiscordLogs:" .. channel:lower() .. ":steamid") ~= "true"
		then
			_steamID = ids.steam
		else
			_steamID = nil
		...
		if
			cfgFile["steamUrl"]
			and GetResourceKvpString("legendsDiscordLogs:" .. channel:lower() .. ":steamurl") ~= "true"
		then
			if ids.steam then
		...
		TriggerEvent(
			"legendsSystems:legendsDiscordLogs:errorLog",
	...
	if
		cfgFile["license"]
		and GetResourceKvpString("legendsDiscordLogs:" .. channel:lower() .. ":license") ~= "true"
	then
	...
	if
		cfgFile["license2"]
		and GetResourceKvpString("legendsDiscordLogs:" .. channel:lower() .. ":license2") ~= "true"
	then
	...
	if cfgFile["ip"] and GetResourceKvpString("legendsDiscordLogs:" .. channel:lower() .. ":ip") ~= "true" then
		_ip = ids.ip:gsub("ip:", "")
	else
		_ip = nil
	end
	...
	if
		cfgFile["playerId"]
		and GetResourceKvpString("legendsDiscordLogs:" .. channel:lower() .. ":playerid") ~= "true"
	then
	...
	if
		cfgFile["playerPing"]
		and GetResourceKvpString("legendsDiscordLogs:" .. channel:lower() .. ":playerping") ~= "true"
	then
	...
	if cfgFile["playerHealth"] or cfgFile["playerArmor"] then
		...
		if
			cfgFile["playerHealth"]
			and GetResourceKvpString("legendsDiscordLogs:" .. channel:lower() .. ":playerhealth") ~= "true"
		then
		...
		if
			cfgFile["playerArmor"]
			and GetResourceKvpString("legendsDiscordLogs:" .. channel:lower() .. ":playerarmor") ~= "true"
		then
